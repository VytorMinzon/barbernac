...
class ClienteController extends Controller
{
    ...
    public function editar($id = null)
    {
        ...
        $clienteExistente = $this->clienteModel->getClienteById($id);
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (!empty($_POST['senha_cliente'])) {
                $dadosCliente['senha'] = ($_POST['senha_cliente']);
            }

            // Upload da imagem se enviada
            if (isset($_FILES['foto_cliente']) && $_FILES['foto_cliente']['error'] === 0) {
                $caminhoFoto = $this->uploadFoto($_FILES['foto_cliente']);
                if ($caminhoFoto) {
                    $dadosCliente['foto_cliente'] = $caminhoFoto;
                }
            }

            $sucesso = $this->clienteModel->atualizarCliente($id, $dadosCliente);
        ...
        $dados = [];
        $dados['cliente'] = $clienteExistente;
