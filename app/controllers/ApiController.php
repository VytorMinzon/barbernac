...
class ApiController extends Controller
{
    ...
    public function buscarCliente($id)
    {
        try {
            ...
            // Ajusta a URL da foto para caminho completo
    ...
    public function criarAgendamento()
    {
        ...
        if (!isset($dados['id_servico'], $dados['id_funcionario'], $dados['data_agendamento'])) {
            http_response_code(400);
            echo json_encode(['erro' => 'Dados obrigatórios ausentes.']);
            return;
        }

        // Inclui o id_cliente do usuário autenticado no array de dados
        $dados['id_cliente'] = $cliente['id'];

        // Define o status do agendamento
        $dados['status_agendamento'] = 'Em análise';

        // Chama o model para criar o agendamento
        $id = $this->clienteModel->criarAgendamento($dados);

        // Retorna resposta de sucesso com o ID do novo agendamento
